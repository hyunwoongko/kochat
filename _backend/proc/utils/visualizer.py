...
class Visualizer:
    ...
    def save_result(self, loss, eval_dict, mode):
        """
    ...
    def draw_f1_score(self):
        """
        f1 score 그래프를 그립니다.
        test가 True인 경우 testing f1 score도 함께 그립니다.
        """

        train_loss = self.__load_txt('train_f1_score')
        plt.plot(train_loss, 'fuchsia', label='train_f1_score')

        if len(self.test_loss) != 0:
            test_loss = self.__load_txt('test_f1_score')
            plt.plot(test_loss, 'purple', label='test_f1_score')

        self.__draw_graph('f1_score')

    def draw_matrix(self, cm, target_names, mode):
        """
        ...
        title_font_size = (label_length * 1.5) + 5
        ...
        plt.title('{} confusion matrix'.format(mode), fontdict={'fontsize': title_font_size}, pad=35)
        thresh = cm.max() / 1.5
    ...
    def draw_report(self, report, mode):
        """
    ...
    def __save_txt(self, array, mode):
        """
