...
class Model(nn.Module):
    ...
    def __init__(self, vector_size, d_model, layers, classes, device):
        super().__init__()
    ...
    def init_hidden(self, batch_size):
        return (
        torch.autograd.Variable(torch.randn(self.layers * self.direction, batch_size, self.d_model).to(self.device)),
        torch.autograd.Variable(torch.randn(self.layers * self.direction, batch_size, self.d_model).to(self.device)))
