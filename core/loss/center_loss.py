...
class CenterLoss(nn.Module):
    ...
    def __init__(self, factor, d_model, classes):
        super(CenterLoss, self).__init__()
        self.classes = classes
        self.factor = factor
        self.feat_dim = d_model
        self.centers = nn.Parameter(torch.randn(self.classes, d_model).cuda())
    ...
    def forward(self, x, labels):
        ...
        # distmat = x^2 + c^2
        distmat = torch.pow(x, 2).sum(dim=1, keepdim=True).expand(batch_size, self.classes) + \
        ...
        # 클래스 마스킹
        classes = torch.arange(self.classes).long().cuda()
        labels = labels.unsqueeze(1).expand(batch_size, self.classes)
        mask = labels.eq(classes.expand(batch_size, self.classes))
