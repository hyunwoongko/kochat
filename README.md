...
- ### 기존 챗봇 빌더와의 차이점
1. 기존에 상용화된 많은 챗봇 빌더와 Kochat은 타깃으로 하는 사용자가 다릅니다.
...
Open domain 챗봇은 주로 잡담 등을 수행하는 심심이 등이 챗봇이 대표적으로 Chit-chat이라고도 불립니다.
요즘 출시되는 시리나 빅스비 같은 인공지능 비서, 인공지능 스피커들은 Open domain 챗봇과 
Close domain 챗봇이 모두 포함되어 있는 경우가 많습니다.
<br><br>
...
먼저 Open domain 챗봇의 경우는 딥러닝 분야에서는 대부분, 신경망 기계번역(Neural Machine Translation) 
모델인 Seq2Seq에서 출발해서 연구되어왔습니다. 최근에 발표된 Google의 
[Meena](https://ai.googleblog.com/2020/01/towards-conversational-agent-that-can.html)
같은 모델을 보면, 모델아키텍처나 복잡한 프레임워크 없이 End to end 모델만으로
...
Close domain 분야는 대부분 Slot filling 방식으로 구현되고 있습니다. 물론 Close domain 챗봇을
End to end로 구현하려는 [다양한](https://arxiv.org/pdf/1605.07683.pdf) 
...
성능이 나오지 않았기 때문에 현업에 적용되기는 어려움이 있습니다. 때문에 대부분의 목적지향 챗봇
애플리케이션의 경우 기존 방식인 slot filling 방식으로 구현되고 있습니다.
<br><br>

- slot filling (슬롯 채우기)

Slot filling 방식은 목적지향 챗봇 구현의 핵심으로서 두가지 개념인 '인텐트(Intent)'와 
'엔티티(Entity)'를 기반으로 만들어집니다. 가장 먼저 우리가 인공지능 스피커를 만들고 있다고 가정해봅시다.


,
그 기능 중 날씨 알림 챗봇을 만든다면, 아무리 많은 데이터를 넣던, 모델 구조를 좋은걸 쓰던, 
어찌됐던간에 기상청 API 등을 사용해서 외부로부터 실시간 정보를 제공받아야만 합니다. 
때문에 우리는 기상청 API에 정보를 요청해야하는데, API마다 다르지만 기상청의 경우에는
리퀘스트를 위해 날짜와 지역이라는 두가지 정보를 제공해야합니다.
